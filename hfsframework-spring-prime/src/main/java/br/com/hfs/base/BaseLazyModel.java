...
public class BaseLazyModel<T, I extends Serializable, B extends BaseService<T, I, ? extends JpaRepository<T, I>>>
	...
	public List<T> load(int offset, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy,
		...
		if (!usarQueryNativa) {
			...
			try {
				Pageable pagination = PageRequest.of(offset, pageSize, Direction.ASC, sortBy.values().toString());
				ds = service.findAll(pagination).toList();
				//ds = service.listByRange(offset + 1, offset + pageSize);
			} catch (IndexOutOfBoundsException e) {
				Pageable pagination = PageRequest.of(offset + 1, offset + (dataSize % pageSize), 
						Direction.ASC, sortBy.values().toString());
				ds = service.findAll(pagination).toList();
